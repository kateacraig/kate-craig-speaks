 ...
 *
 * If a Contact Form 7 shortcode is set (Appearance → Customize → Kate Craig
 * Speaking) and CF7 is active, the booking form is rendered via CF7 — styled
 * to match, with the gradient "Thank you" panel shown on the wpcf7mailsent
 * event. Otherwise it falls back to the built-in form (admin-post + wp_mail).
 *
...
$cf7_shortcode = kcs_get_option( 'kcs_cf7_shortcode', '' );
$use_cf7       = ( '' !== $cf7_shortcode ) && shortcode_exists( 'contact-form-7' );
...
<section id="contact" style="scroll-margin-top:84px;background:var(--bg2);padding:84px 24px;transition:background .45s;border-top:1px solid rgba(255,255,255,0.5)">
	<div style="max-width:900px;margin:0 auto">
		<div style="text-align:center;max-width:600px;margin:0 auto">
			<div style="display:inline-flex;align-items:center;font-size:12.5px;letter-spacing:var(--eyebrow-spacing);text-transform:uppercase;font-weight:700;color:var(--blue)">Book Kate</div>
		...
		<?php if ( $use_cf7 ) : ?>
			...
			<div class="kc-cf7" style="margin-top:38px;background:var(--bg);border:1px solid var(--line);border-radius:var(--radius-lg);padding:32px;box-shadow:var(--shadow)">
				<?php echo do_shortcode( $cf7_shortcode ); ?>
				<div style="margin-top:8px;font-size:14px;color:var(--muted)">Prefer email? <a href="mailto:<?php echo esc_attr( $kcs_email ); ?>" style="color:var(--blue);font-weight:700;text-decoration:none"><?php echo esc_html( $kcs_email ); ?></a></div>
			</div>

			<!-- Revealed by assets/js/contact-cf7.js on the wpcf7mailsent event. -->
			<div id="kc-thanks" role="status" style="display:none;margin-top:38px;background:var(--grad);color:#fff;border-radius:var(--radius-lg);padding:48px 32px;text-align:center;box-shadow:var(--shadow);animation:kc-rise .4s ease">
